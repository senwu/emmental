...
def test_pad_batch(caplog):
    ...
    assert torch.equal(mask_batch, torch.BoolTensor([[0, 0], [0, 1], [0, 0]]))
    ...
    assert torch.equal(mask_batch, torch.BoolTensor([[1, 0, 0], [1, 1, 0], [0, 0, 0]]))
    ...
    assert torch.equal(mask_batch, torch.BoolTensor([[0, 0], [1, 0], [0, 0]]))

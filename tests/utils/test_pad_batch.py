...
def test_pad_batch(caplog):
    ...
    assert torch.equal(mask_batch, torch.Tensor([[0, 0, 1], [0, 1, 1], [0, 0, 0]]))

    padded_batch, mask_batch = pad_batch(batch, max_len=2)

    assert torch.equal(padded_batch, torch.Tensor([[1, 2], [3, 0], [4, 5]]))
    assert torch.equal(mask_batch, torch.Tensor([[0, 0], [0, 1], [0, 0]]))
    ...
    assert torch.equal(mask_batch, torch.Tensor([[1, 0, 0], [1, 1, 0], [0, 0, 0]]))
    ...
    assert torch.equal(mask_batch, torch.Tensor([[0, 0], [1, 0], [0, 0]]))
